...
const penner = (() => {
  ...
  Object.keys(functionEasings).forEach((name) => {
    ...
    eases['easeOutIn' + name] = (a, b) => (t) =>
      t < 0.5 ? (1 - easeIn(a, b)(1 - t * 2)) / 2 : (easeIn(a, b)(t * 2 - 1) + 1) / 2
  })
...
function flattenKeyframes(keyframes) {
  const propertyNames = filterArray(flattenArray(keyframes.map((key) => Object.keys(key))), (p) => is.key(p)).reduce(
    (a, b) => {
...
function getInstanceTimings(animations, tweenSettings) {
  ...
  timings.endDelay = animLength
    ? timings.duration -
      Math.max.apply(
        Math,
        animations.map((anim) => getTlOffset(anim) + anim.duration - anim.endDelay)
      )
  ...
  return (el, i, t) => {
    ...
    if (!values.length) {
      ...
      if (direction === 'reverse')
        values = values.map((val) => (axis ? (val < 0 ? val * -1 : -val) : Math.abs(maxValue - val)))
    }
