...
export function equal(v0, v1) {
  if (Array.isArray(v0)) return v0.every((val, index) => val === v1[index])
  return v0 === v1
}

export function each(array, iterator) {
  Array.isArray(array) ? array.forEach(iterator) : iterator(array, -1)
}

export function map(array, iterator) {
  return Array.isArray(array) ? array.map(iterator) : iterator(array, -1)
}
